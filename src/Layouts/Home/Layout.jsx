...
export default function Layout({ title, breads = [], titleRight, header, gradient = false, showSidebar = true, onlyAdmin = false, children }) {
        ...
        <Box ml={{ base: 0, md: showSidebar ? 60 : 0 }}>
          ...
          <Box as="main" bg={header ? 'white' : 'gray.50'}>
            ...
            <Box minH={'74vh'} px={layoutWidth} pb={16}>
              {(onlyAdmin == true && authUser?.business_role == 'admin') || onlyAdmin == false ? children : <Center py={20}>
                <Text>You are not authorised to see this page!</Text>
              </Center>}
            </Box>
